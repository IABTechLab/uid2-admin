...
public class SaltRotationTest {
    ...
    void testRefreshFromCalculation(int lastRotationDaysAgo, int expectedRefreshFromDaysPostLastRotation) throws Exception {
        var lastRotation = targetDateAsInstant.minus(lastRotationDaysAgo, DAYS);
        RotatingSaltProvider.SaltSnapshot lastSnapshot = SnapshotBuilder.start()
                .withEntries(
                        new SaltEntry(1, "1", lastRotation.toEpochMilli(), "salt1", 100L, null, null, null)
                )
                .build(oneDayEarlier, sixDaysLater);
        ...
        var expected = lastRotation.plus(expectedRefreshFromDaysPostLastRotation, DAYS).toEpochMilli();
