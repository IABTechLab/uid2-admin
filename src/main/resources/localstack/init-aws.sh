...
# Create KMS RSA key with custom ID and custom key material for JWT signing
# Using _custom_id_ and _custom_key_material_ tags per https://docs.localstack.cloud/aws/services/kms/
# The key material is the RSA private key in PKCS#8 DER format (base64 encoded, same as PEM content)
echo "Creating KMS RSA key with custom ID and key material..."

# RSA private key (PKCS#8 format, base64 encoded - matches the public key in config)
